...
linux_commit_checkout="$(git rev-parse --short "$(git ls-remote "$linux_repo" --tags "$linux_kernel_version_tag" --short | grep -o '^[a-zA-Z0-9]*')")"
...
git checkout "$linux_commit_checkout"
...
# LINENO: ${LINENO}"
    ...
    git checkout "$zfs_commit_checkout" 
fi
